 ...
 *
 * Usage:
 *   tsx test-processor.ts --aptos-core /path/to/aptos-core --processor-root /path/to/processor [--skip-localnet]
 *
 * This script:
 *   1. Starts a localnet in a temp directory (unless --skip-localnet)
...
function parseArgs() {
  ...
  if (!aptosCorePath || !processorRoot) {
    console.error(
      "Usage: tsx test-processor.ts --aptos-core /path/to/aptos-core --processor-root /path/to/processor [--skip-localnet]",
    );
    process.exit(1);
  }
...
const { aptosCorePath, processorRoot, skipLocalnet } = parseArgs();
...
const TEST_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "ca-test-"));
...
async function generateOnChainActivity(aptos: Aptos, harness: Harness): Promise<string[]> {
  ...
  // Some SDK operations use WASM range proofs that may crash on certain
  // environments. Wrap each in try/catch so the test can verify whatever
  // events we manage to generate.
  ...
  try {
    ...
    console.log(`   Normalize skipped (WASM error): ${e.message}`);
    skippedEvents.push("Normalized");
  }
  ...
  try {
    ...
    console.log(`   Transfer skipped (WASM error): ${e.message}`);
    skippedEvents.push("Transferred");
  }
  ...
  try {
    ...
    console.log(`   Withdraw skipped (WASM error): ${e.message}`);
    skippedEvents.push("Withdrawn");
  }
  ...
  try {
    ...
    console.log(`   Key rotation skipped (WASM error): ${e.message}`);
    skippedEvents.push("KeyRotated", "IncomingTransfersPauseChanged");
  }
  ...
  if (skippedEvents.length > 0) {
    console.log(`WARNING: Skipped events due to WASM errors: ${skippedEvents.join(", ")}`);
  }
...
function verify(rows: any[], skippedEvents: string[]) {
  ...
  const expectedEventTypes = allEventTypes.filter((t) => !skippedSet.has(t));
  ...
  if (skippedEvents.length > 0) {
    console.log(
      `${expectedEventTypes.length}/${allEventTypes.length} event types present (${skippedEvents.length} skipped).`,
    );
  } else {
  ...
  if (!skippedSet.has("Deposited")) {
    ...
    // The first deposit should be Alice depositing 100.
  ...
  // Normalized: exactly 1 if not skipped.
  ...
  // Transferred: exactly 1 if not skipped.
  if (!skippedSet.has("Transferred")) {
    ...
    const d = eventData(transferred[0]);
    assert(Array.isArray(d.amount_P), "Transferred missing amount_P array");
    assert(Array.isArray(d.amount_R_sender), "Transferred missing amount_R_sender");
    assert(Array.isArray(d.amount_R_recip), "Transferred missing amount_R_recip");
    assert(typeof d.memo === "string", "Transferred missing memo");
    console.log("Transferred: 1 event, Alice->Bob, encrypted amounts present.");
  }

  // Withdrawn: exactly 1 if not skipped.
  if (!skippedSet.has("Withdrawn")) {
  ...
  // KeyRotated: exactly 1 if not skipped, with new_ek.
  if (!skippedSet.has("KeyRotated")) {
    ...
    const d = eventData(keyRotated[0]);
    assert(d.new_ek && d.new_ek.data, "KeyRotated missing new_ek");
    console.log("KeyRotated: 1 event, Alice, new_ek present.");
  }

  // IncomingTransfersPauseChanged: at least 1 if not skipped.
  if (!skippedSet.has("IncomingTransfersPauseChanged")) {
  ...
  for (const et of expectedEventTypes) {
    const count = rows.filter((r: any) => r.event_type === et).length;
...
async function main() {
  ...
  try {
    ...
    if (!skipLocalnet) {
      ...
      log("Skipping localnet start (--skip-localnet)");
    ...
    const skippedEvents = await generateOnChainActivity(aptos, harness);

    // Step 5: Start the processor and let it index.
    await startProcessor();
    log("Waiting for processor to index all transactions");

    // We expect at least 6 distinct event types from governance + basic user ops.
    const minExpectedTypes = 6;
